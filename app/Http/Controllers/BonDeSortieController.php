...
use App\Http\Requests\BonDeSortieRequest;
...
class BonDeSortieController extends Controller
{
    ...
    {
        $bons = BonDeSortie::orderBy('date', 'desc')->orderBy('id', 'asc')->get();
        $latestBon = $bons->first();
        $agence = Agence::first();

        return view('bon_sortie.index', compact('bons', 'latestBon', 'agence'));
    }
    ...
    public function create()
    {
        ...
        return view('bon_sortie.create', compact('clients', 'articles', 'agence','categories','fournisseurs','marques'));
    }
    ...
    public function store(Request $request)
    {
        ...
        'num' => [
            'required',
            'string',
            \Illuminate\Validation\Rule::unique('bon_de_sortie')->where(function ($query) use ($request) {
    ...
    try {
        ...
        DB::commit();
        return redirect()->route('bon_sortie.index')->with('info', 'bon de Sortie créé avec succès.');
    } catch (\Exception $e) {
        DB::rollBack();
        \Illuminate\Support\Facades\Log::error("Error in BonDeSortieController@store: " . $e->getMessage());
        return back()->with('error', 'Une erreur est survenue lors de l\'enregistrement du bon de sortie. Veuillez réessayer.');
    }
}
    ...
    $standardArticles = DB::table('bon_sortie_article')
        ->where('bon_id', $bon->id)
        ->join('articles', 'bon_sortie_article.article_id', '=', 'articles.id')
        ->select('articles.id', 'articles.designation as produit', 'bon_sortie_article.quantite as quantite', 'bon_sortie_article.prix_article as prix')
        ->get()
        ->toArray();
    
    // Get custom articles
    $customArticles = DB::table('bon_sortie_custom_articles')
        ->where('bon_id', $bon->id)
    ...
    $bonArticles = array_merge($standardArticles, $customArticles);
        ...
        return view('bon_sortie.edit', compact('bon',  'clients' ,'articles', 'bonArticles' , 'categories','fournisseurs','marques'));
    }
...
public function update(Request $request, $id)
{
    ...
    try {
       // Handle bon num formatting
        ...
        DB::table('bon_sortie_article')->where('bon_id', $bon->id)->delete();
        DB::table('bon_sortie_custom_articles')->where('bon_id', $bon->id)->delete();
        ...
        DB::commit();
        return redirect()->route('bon_sortie.index')->with('info', 'bon de sortie mis à jour avec succès.');
    } catch (\Exception $e) {
        DB::rollBack();
        \Illuminate\Support\Facades\Log::error("Error in BonDeSortieController@update: " . $e->getMessage());
        return back()->withInput()->with('error', 'Une erreur est survenue lors de la mise à jour du bon de sortie.');
    }
}
public function show(string $id)
{
    $bon = BonDeSortie::with('client')->findOrFail($id);
    ...
    return view('bon_sortie.show', compact('agences', 'bonArticles', 'bon'));
}
    ...
    public function destroy($id)
    {
        try {
            $bon = BonDeSortie::findOrFail($id);
            $bon->delete();
            return redirect()->route('bon_sortie.index')->with('success', 'Bon de sortie supprimé avec succès.');
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error("Error in BonDeSortieController@destroy: " . $e->getMessage());
            return back()->with('error', 'Une erreur est survenue lors de la suppression du bon de sortie.');
        }
    }
    ...
    public function generatePDF(string $id)
    {
        ...
        $html = view('bon_sortie.pdf', $data)->render();
        ...
        $filename = "Bon de Sortie " . str_replace('/', ' ', $data['numero']);
    ...
    {
        $data = $this->getBonData($id);
        return view('bon_sortie.download', $data);
    }
