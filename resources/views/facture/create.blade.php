...
if ($lastFacture) {
    $lastNumPrefix = (int) explode('/', $lastFacture)[0];
...
<section class="section mt-4">
        ...
        <div class="col-lg-12">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    <form id="factureForm" method="POST" action="{{ route('factures.store') }}" class="row g-3" onsubmit="return prepareForm()">
                        @csrf
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control @error('num') is-invalid @enderror" id="num" name="num" 
                                   value="{{ old('num', $nextNum) }}" >
                            @error('num')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="d-flex gap-2 align-items-start mb-2" id="firstPaymentRow">
                                <div class="flex-fill">
                                    <select class="form-select @error('paiement') is-invalid @enderror"
                                            name="paiement" id="inputPaiement"
                                            onchange="toggleMainCheque()">
                                        <option value="" hidden>-- Méthode --</option>
                                        @foreach ($paiements as $paiement)
                                            <option value="{{ $paiement['id'] }}"
                                                @selected(old('paiement') == $paiement['id'])>
                                                {{ $paiement['methode'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="flex-fill">
                                    <input type="number" step="0.000001" min="0"
                                           class="form-control"
                                           id="mainMontant" name="montantPaiy"
                                           value="{{ old('montantPaiy', 0) }}"
                                           placeholder="Montant">
                                </div>
                                <div class="flex-fill" id="mainChequeField" style="display:none;">
                                    <input type="text" class="form-control"
                                           id="numero_cheque" name="numero_cheque"
                                           value="{{ old('numero_cheque') }}"
                                           placeholder="N° Chèque">
                                </div>
                            </div>

                            {{-- Extra payment rows added dynamically --}}
                            <div id="reglementsContainer"></div>

                            <button type="button" class="btn btn-sm btn-outline-primary mt-1"
                                    onclick="addReglementRow()">
                                <i class="bi bi-plus"></i> Ajouter un autre paiement
                            </button>
                            <input type="hidden" name="reglements" id="reglementsInput" value="[]">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('etat') is-invalid @enderror" id="etat" name="etat" required>
                                <option value="0" {{ old('etat', '0') == '0' ? 'selected' : '' }}>Impayée</option>
                                <option value="1" {{ old('etat') == '1' ? 'selected' : '' }}>Payée</option>
                            </select>
                            @error('etat')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" 
                                       id="tva" name="tva" {{ old('tva', 1) ? 'checked' : '' }}>
                                <label class="form-check-label" for="tva">TVA inclus</label>
                            </div>
                            <input type="number" class="form-control" id="inputTva" 
                                   name="taux" value="{{ old('taux', old('tva', 1) ? 20 : 0) }}" readonly>
                        </div>
  <!-- Products Table Section -->
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-3">
                                <select name="etatremise" id="remise" class="form-select">
                                    <option value="off">Aucune remise</option>
                                    <option value="pourcentage">Pourcentage</option>
                                    <option value="montant">Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-2">
                                <input type="number" step="1" id="InpRemis" name="remise" 
                                       class="form-control" value="{{ old('remise', 0) }}" min="0">
                            </div>
                        </div>
                        ...
                        <div class="text-center mt-5">
                           <a href="{{ route('facture.index') }}" class="btn btn-secondary">Retour</a>
...
<script>
    ...
    // Payment amount toggle functionality
    if ($('#typeMpaiy').prop("checked") == true) {
        $('#inputtypeMpaiy').removeAttr('readonly')
        $("#inputtypeMpaiy").css("background-color", "#fff");
    }

    if ($('#typeMpaiy').prop("checked") == false) {
        $('#inputtypeMpaiy').attr('readonly', 'readonly')
        $("#inputtypeMpaiy").css("background-color", "#B5C0D0");
        $('#inputtypeMpaiy').val(0)
    }

    $('#typeMpaiy').click(function() {
    ...
    $("#inputtypeMpaiy").on("change", function() {
        if (parseInt(motantTotal.value) >= parseInt($("#inputtypeMpaiy").val())) {
            $('#inputtypeMpaiy').removeClass('is-invalid');
            $('#inputtypeMpaiy').addClass('is-valid');
            $('#inputtypeMpaiy').next('div').hide();
        } else {
            $('#inputtypeMpaiy').removeClass('is-valid');
            $('#inputtypeMpaiy').addClass('is-invalid');
        }
    });
    ...
    // Add changed flag to existing products
    products = products.map(product => ({
        ...product,
        changed: false
    }));

    // Initialize with one empty row if no products exist
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('inputPaiement').addEventListener('change', toggleMainCheque);
        toggleMainCheque();
        
        @if(old('articles'))
            products = @json(json_decode(old('articles'), true));
            // Re-add changed flag for old products
            products = products.map(product => ({
                ...product,
                changed: false
            }));
        ...
        // Set old values for select fields
        @if(old('typeMpaiy'))
            document.getElementById('typeMpaiy').value = '{{ old('typeMpaiy') }}';
        @endif
        
        @if(old('etat'))
            document.getElementById('etat').value = '{{ old('etat') }}';
        @endif
        
        @if(old('etatremise'))
            document.getElementById('remise').value = '{{ old('etatremise') }}';
        @endif
        
        @if(old('remise'))
            document.getElementById('InpRemis').value = '{{ old('remise') }}';
        @endif

        // Trigger initial state
        validateDiscount();

        // Restore old reglements if any
        @if(old('reglements'))
            let oldReglements = @json(old('reglements'));
            if (typeof oldReglements === 'string') {
                try {
                    oldReglements = JSON.parse(oldReglements);
                } catch (e) {
                    oldReglements = [];
                }
            }
            if (Array.isArray(oldReglements)) {
                oldReglements.forEach(r => {
                    addReglementRow(r);
                });
            }
        @endif
    ...
    function convertTtcToHt(index, inputElement) {
        ...
        if (ttcValue > 0) {
            ...
            const preciseValue = Number(htValue.toFixed(6));
            inputElement.value = htValue.toFixed(2);
            updateProduct(index, 'prix', preciseValue);
        } else {
            // If value is 0, use the value as is
        ...
        if (discountType === 'off') {
            ...
            if (discountType === 'pourcentage' && discountValue > 100) {
                ...
                if (typeof showAlert === 'function') {
                    showAlert('Le pourcentage de remise ne peut pas dépasser 100%', 'warning');
                } else {
    ...
    // Toggle cheque field on the main (first) payment row
    function toggleMainCheque() {
        const select = document.getElementById('inputPaiement');
        const chequeField = document.getElementById('mainChequeField');
        const selectedText = select.options[select.selectedIndex]?.text.toLowerCase() ?? '';
        chequeField.style.display = (selectedText.includes('chèque') || selectedText.includes('cheque')) ? 'flex' : 'none';
    }

    // Toggle cheque field (legacy reference kept for compatibility)
    function toggleChequeField() {
        toggleMainCheque();
    }

    const paiementsOptions = {!! json_encode($paiements) !!};
    let reglementIndex = 0;
    
    function addReglementRow(reglement = null) {
        reglementIndex++;
        const container = document.getElementById('reglementsContainer');
        
        let optionsHtml = '<option value="" hidden>-- Séléctionner --</option>';
        paiementsOptions.forEach(p => {
            const isSelected = reglement && reglement.paiement_id == p.id ? 'selected' : '';
            optionsHtml += '<option value="' + p.id + '" ' + isSelected + '>' + p.methode + '</option>';
        });

        const montantVal = reglement ? reglement.montant : '';
        const chequeVal = reglement && reglement.num_cheque ? reglement.num_cheque : '';
        const chequeDisplay = chequeVal ? 'block' : 'none';

        const rowHtml = `
            <div class="row mb-2 reglement-row" id="reglement_row_${reglementIndex}">
                <div class="col-11">
                    <div class="d-flex gap-2">
                        <select class="form-select reglement-paiement" required onchange="toggleReglementCheque(${reglementIndex})">
                            ${optionsHtml}
                        </select>
                        <input type="number" step="0.000001" class="form-control reglement-montant" value="${montantVal}" required placeholder="Montant">
                        <input type="text" class="form-control reglement-cheque" value="${chequeVal}" placeholder="N° de Chèque" style="display: ${chequeDisplay};">
                    </div>
                </div>
                <div class="col-1 d-flex align-items-center">
                    <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeReglementRow(${reglementIndex})">
                        <i class="bi bi-x"></i>
                    </button>
                </div>
            </div>
    ...
    function removeReglementRow(index) {
        document.getElementById('reglement_row_' + index).remove();
    }

    function toggleReglementCheque(index) {
        const row = document.getElementById('reglement_row_' + index);
        const select = row.querySelector('.reglement-paiement');
        const chequeInput = row.querySelector('.reglement-cheque');
        const selectedText = select.options[select.selectedIndex].text.toLowerCase();
        
        if (selectedText.includes('chèque') || selectedText.includes('cheque')) {
    ...
    function prepareForm() {
        ...
        const filteredProducts = products.filter(product => 
            product.produit && product.produit.trim() !== '' && product.quantite > 0
        );
        ...
        // Update all calculated fields one last time
        calculateTotals();
        
        // Set the articles JSON
        document.getElementById('articlesInput').value = JSON.stringify(filteredProducts);
        

        // No validation needed for montantPaiy — reglements array is the source of truth

        // Collect reglements
        const reglementsData = [];
        document.querySelectorAll('.reglement-row').forEach(row => {
            const paiementId = row.querySelector('.reglement-paiement').value;
            const montant = row.querySelector('.reglement-montant').value;
            const numCheque = row.querySelector('.reglement-cheque').value;
            if (paiementId && montant) {
                reglementsData.push({
        ...
        document.getElementById('reglementsInput').value = JSON.stringify(reglementsData);
