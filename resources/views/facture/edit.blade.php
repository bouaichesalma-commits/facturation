...
<section class="section mt-4">
        ...
        <div class="col-lg-12">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    <form id="factureForm" method="POST" action="{{ route('facture.update', $facture->id) }}" class="row g-3" onsubmit="return prepareForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control @error('num') is-invalid @enderror" 
                                   id="inputNum" name="num" value="{{ old('num', $facture->num)  }}">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" class="form-control" id="date" name="date" 
                                   value="{{ old('date', default: $facture->date) }}" required>
                        </div>
                        ...
                        <div class="col-md-8">
                            ...
                            {{-- Existing reglements load below via JS from DB --}}
                            ...
                            {{-- Keep paiement hidden for updating facture.paiement_id --}}
                            <input type="hidden" name="paiement" id="inputPaiement"
                                   value="{{ old('paiement', $facture->paiement_id) }}">
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" 
                                       id="tva" name="tva" {{ (old('tva', $facture->tva) === 'on' || old('tva', $facture->tva) == 1) ? 'checked' : '' }}>
                                <label class="form-check-label" for="tva">TVA inclus</label>
                            </div>
                            <input type="number" class="form-control" id="inputTva" 
                                   name="taux" value="{{ old('taux', (old('tva', $facture->tva) === 'on' || old('tva', $facture->tva) == 1) ? 20 : 0) }}" readonly >
                        </div>
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-3">
                                <select name="etatremise" id="remise" class="form-select">
                                    <option value="off" {{ old('etatremise', $facture->etatremise) == 'off' ? 'selected' : '' }}>Aucune remise</option>
                                    <option value="pourcentage" {{ old('etatremise', $facture->etatremise) == 'pourcentage' ? 'selected' : '' }}>Pourcentage</option>
                                    <option value="montant" {{ old('etatremise', $facture->etatremise) == 'montant' ? 'selected' : '' }}>Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-2">
                                <input type="number" step="1" id="InpRemis" name="remise" 
                                       class="form-control" value="{{ old('remise', $facture->remise) }}" min="0">
                            </div>
                        </div>
 ...
 <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        calculateTotals();
        setupEventListeners();
        filterProducts(); // Initial filter for modal
        ...
        let existingReglements = @json(old('reglements', $facture->reglements()->with('paiement')->get()));
        ...
        @if(old('remise', $facture->remise))
            document.getElementById('InpRemis').value = '{{ old('remise', $facture->remise) }}';
        @endif
    ...
    function addReglementRow(reglement = null) {
        ...
        toggleReglementCheque(reglementIndex); // Ensure visibility matches initial selection
    }
    ...
    function prepareForm() {
        ...
        // No validation needed — reglements array is the source of truth
