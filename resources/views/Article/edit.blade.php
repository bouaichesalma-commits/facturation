        ...
        <div class="col-lg-8">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('article.update', $Article['id']) }}" class="row g-3" id="ArticleForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="prix" value="{{ old('prix', $Article->prix) }}" 
                                ...
                                id="inputPrix" placeholder="Veuillez saisir ici ...">
                            @error('prix')
                                <div class="text-danger ps-3">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="categorieproduit_id" class="form-control @error('categorieproduit_id') is-invalid @enderror" id="inputCategorieProduit">
                                ...
                                @foreach ($categories as $categorie)
                                    <option value="{{ $categorie->id }}" 
                                        {{ old('categorieproduit_id', $Article->categorieproduit_id) == $categorie->id ? 'selected' : '' }}>
                                        {{ $categorie->categorie }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="fournisseur_id" class="form-control @error('fournisseur_id') is-invalid @enderror" id="inputfournisseurs">
                                ...
                                @foreach ($fournisseurs as $fournisseur)
                                    <option value="{{ $fournisseur->id }}" 
                                        {{ old('fournisseur_id', $Article->fournisseur_id) == $fournisseur->id ? 'selected' : '' }}>
                                        {{ $fournisseur->nom }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="marque_id" class="form-control @error('marque_id') is-invalid @enderror" id="inputmarques">
                                ...
                                @foreach ($marques as $marque)
                                    <option value="{{ $marque->id }}" 
                                        {{ old('marque_id', $Article->marque_id) == $marque->id ? 'selected' : '' }}>
                                        {{ $marque->nom }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="content">
                                ...
                                <span class="form-span">
                                    ...
                                    <input type="hidden" spellcheck="false" name="Details" 
                                           value="{{ old('Details', $Article->Details) }}"  
                        ...
                        <div class="text-center mt-5">
                            <a href="{{ route('article.index') }}" class="btn btn-secondary">@lang('messages.Retour')</a>
                            <button type="submit" class="btn btn-primary" onclick="prepareTags(event)" id="updatearticle">@lang('Modifier')</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    @if($Article->Details)
        let detailsContent = {!! json_encode($Article->Details) !!};
        if (detailsContent) {
    ...
    function prepareTags(event) {
        ...
        const updateBtn = document.getElementById('updatearticle');
