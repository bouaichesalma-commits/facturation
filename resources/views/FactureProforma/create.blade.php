...
$lastFacture = DB::table('facture_proformas')
        ...
        <div class="col-lg-12">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    <form id="factureForm" method="POST" action="{{ route('factureProforma.store') }}" class="row g-3" onsubmit="return prepareForm()">
                        @csrf
                        ...
                        <div class="col-md-4">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" 
                                       ...
                                       {{ old('tva', 1) == 1 ? 'checked' : '' }} onchange="updateTvaState()">
                                <label class="form-check-label" for="tva">TVA inclus</label>
                            </div>
                            <input type="number" class="form-control @error('taux') is-invalid @enderror" id="taux" 
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-3">
                                <select name="etatremise" id="remise" class="form-select">
                                    <option value="off" {{ old('etatremise') == 'off' ? 'selected' : '' }}>Aucune remise</option>
                                    <option value="pourcentage" {{ old('etatremise') == 'pourcentage' ? 'selected' : '' }}>Pourcentage</option>
                                    <option value="montant" {{ old('etatremise') == 'montant' ? 'selected' : '' }}>Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-2">
                                <input type="number" id="InpRemis" name="remise" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Add event listener for TVA toggle
        document.getElementById('tva').addEventListener('change', calculateTotals);
        filterProducts();
        updateTvaState();
    ...
    function setupEventListeners() {
        ...
        document.getElementById('taux').addEventListener('input', calculateTotals);
    ...
    function convertTtcToHt(index, inputElement) {
        ...
        const tvaRate = parseFloat(document.getElementById('taux').value) / 100;
    ...
    // Function to toggle cheque number field
    function toggleChequeField() {
        const paiementSelect = document.getElementById('inputPaiement');
        const chequeField = document.getElementById('chequeField');
        if (!paiementSelect || !chequeField) return;
        
        if (paiementSelect.selectedIndex === -1) return;
        const selectedText = paiementSelect.options[paiementSelect.selectedIndex].text.toLowerCase();
    ...
    function toggleMainCheque() {
        const sel = document.getElementById('inputPaiement');
        const chequeField = document.getElementById('mainChequeField');
        const text = sel.options[sel.selectedIndex]?.text?.toLowerCase() ?? '';
        chequeField.style.display = (text.includes('ch\u00e8que') || text.includes('cheque')) ? 'block' : 'none';
    }
</script>
@endsection