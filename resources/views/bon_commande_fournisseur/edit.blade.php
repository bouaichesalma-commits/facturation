...
@php
    ...
    if ($articlesOld && is_string($articlesOld)) {
        ...
        $oldProduct = $bonCommandeArticles;
    }
    $tvaChecked = old('tva', $bon_commande_fournisseur->tva == '1');
@endphp
...
<section class="section mt-4">
        ...
        <div class="col-lg-12">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    <form id="factureForms" method="POST" action="{{ route('bon_commande_fournisseur.update', $bon_commande_fournisseur->id) }}" class="row g-3" onsubmit="return prepareForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-6">
                                ...
                                <select class="form-select @error('fournisseur') is-invalid @enderror" id="fournisseur" name="fournisseur" data-placeholder="Séléctionner le fournisseur">
                                    <option value="" hidden>-- Séléctionner le fournisseur --</option>
                                    @foreach ($fournisseurs as $fournisseur)
                                        <option @selected(old('fournisseur') ? old('fournisseur') == $fournisseur['id'] : $bon_commande_fournisseur['fournisseur']['id'] == $fournisseur['id']) value="{{ $fournisseur['id'] }}">{{ $fournisseur['nom'] }}</option>
                                    @endforeach
                                </select>
                                @error('fournisseur')
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control @error('num') is-invalid @enderror" 
                                   id="inputNum" name="num" value="{{ old('num', $bon_commande_fournisseur->num) }}" >
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" class="form-control @error('date') is-invalid @enderror" 
                                   id="inputDate" name="date" value="{{ old('date', $bon_commande_fournisseur->date) }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" 
                                       id="tva" name="tva" {{ (old('tva', $bon_commande_fournisseur->tva) === 'on' || old('tva', $bon_commande_fournisseur->tva) == 1) ? 'checked' : '' }}>
                                <label class="form-check-label" for="tva">TVA inclus</label>
                            </div>
                            <input type="number" class="form-control" id="inputTva" 
                                   name="taux" value="{{ old('taux', (old('tva', $bon_commande_fournisseur->tva) === 'on' || old('tva', $bon_commande_fournisseur->tva) == 1) ? 20 : 0) }}"  readonly>
                        </div>
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-3">
                                <select name="etatremise" id="remise" class="form-select">
                                    <option value="off" {{ old('etatremise', $bon_commande_fournisseur->etatremise) == 'off' ? 'selected' : '' }}>Aucune remise</option>
                                    <option value="pourcentage" {{ old('etatremise', $bon_commande_fournisseur->etatremise) == 'pourcentage' ? 'selected' : '' }}>Pourcentage</option>
                                    <option value="montant" {{ old('etatremise', $bon_commande_fournisseur->etatremise) == 'montant' ? 'selected' : '' }}>Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-2">
                                <input type="number" step="1" id="InpRemis" name="remise" 
                                       class="form-control" value="{{ old('remise', $bon_commande_fournisseur->remise) }}" min="0">
                            </div>
                        </div>
                        ...
                        <table class="table mt-4">
                            <tr>
                                ...
                                <td class="border-0 text-end"><span id="montant-total">{{ number_format($bon_commande_fournisseur->montant, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="montant" id="montant" value="{{ number_format($bon_commande_fournisseur->montant, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">Remise :</th>
                                <td class="border-0 text-end"><span id="remis">{{ number_format($bon_commande_fournisseur->remise, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="montantRemise" id="motantRemise" value="{{ number_format($bon_commande_fournisseur->remise, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">TVA :</th>
                                <td class="border-0 text-end"><span id="tvaa">{{ number_format($bon_commande_fournisseur->taux * $bon_commande_fournisseur->montant / 100, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="valeurTVA" id="motantTVA" value="{{ number_format($bon_commande_fournisseur->taux * $bon_commande_fournisseur->montant / 100, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">Total TTC :</th>
                                <td class="border-0 text-end"><span id="montant-ttc">{{ number_format($bon_commande_fournisseur->montant + ($bon_commande_fournisseur->taux * $bon_commande_fournisseur->montant / 100), 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="totalTTC" id="motantTotal" value="{{ number_format($bon_commande_fournisseur->montant + ($bon_commande_fournisseur->taux * $bon_commande_fournisseur->montant / 100), 2, '.', '') }}">
                            </tr>
                        </table>

                        <!-- Hidden Fields -->
                        <input type="hidden" name="articles" id="articlesInput" value="{{ json_encode($bonCommandeArticles) }}">

                        <div class="text-center mt-5">
                            <a href="{{ route('bon_commande_fournisseur.index') }}" class="btn btn-secondary">Retour</a>
 ...
 <script>
    ...
    function addProductFromModal(article) {
        // Check for duplicate catalog product
        const isDuplicate = products.some(p => 
            p.id === article.id || 
            p.produit.trim().toLowerCase() === article.designation.trim().toLowerCase()
        );
    ...
    function updateProduct(index, field, value) {
        if (field === 'quantite') {
            products[index][field] = Math.max(1, parseInt(value) || 1);
        } else if (field === 'prix') {
            products[index][field] = Math.max(0, parseFloat(value) || 0);
        }
    ...
    function updateProductName(index, value) {
        ...
        const duplicateRow = products.find((p, i) => 
            i !== index && 
            p.produit.trim().toLowerCase() === newNameLower
        );
        
        if (duplicateRow) {
            const rowNum = products.indexOf(duplicateRow) + 1;
            alert(`Un produit avec ce nom existe déjà (Ligne ${rowNum})`);
            renderProductsTable();
            return;
        }
        
        products[index].produit = newName;
        calculateTotals();
    }
    ...
    function prepareForm() {
        ...
        for (const product of filteredProducts) {
            const key = product.id ? `id:${product.id}` : `name:${product.produit.trim().toLowerCase()}`;
            
            if (seen.has(key)) {
                const msg = product.id ? 
                    'Ce produit est en double' : 
                    `Le produit "${product.produit}" est en double`;
                alert(msg);
                return false;
            }
