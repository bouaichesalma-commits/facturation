...
@php
    ...
    if ($articlesOld && is_string($articlesOld)) {
        ...
        $oldProduct = $bonArticles;
    }
    $tvaChecked = old('tva', $bon->tva == '1');
@endphp
        ...
        <div class="col-lg-12">
            <div class="card p-md-3 p-lg-4">
                <div class="card-body">
                    ...
                    <form id="factureForms" method="POST" action="{{ route('bon_retour.update', $bon->id) }}" class="row g-3" onsubmit="return prepareForm()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-9">
                            ...
                            <select class="form-select @error('client') is-invalid @enderror" id="client" name="client">
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client', $bon->client_id) == $client->id ? 'selected' : '' }}>
                                        {{ $client->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control @error('num') is-invalid @enderror" 
                                   id="inputNum" name="num" value="{{ old('num', $bon->num) }}" >
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control @error('date') is-invalid @enderror" 
                                   id="inputDate" name="date" value="{{ old('date', $bon->date) }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" 
                                       id="tva" name="tva" {{ (old('tva', $bon->tva) === 'on' || old('tva', $bon->tva) == 1) ? 'checked' : '' }}>
                                <label class="form-check-label" for="tva">TVA inclus</label>
                            </div>
                            <input type="number" class="form-control" id="inputTva" 
                                   name="taux" value="{{ old('taux', (old('tva', $bon->tva) === 'on' || old('tva', $bon->tva) == 1) ? 20 : 0) }}" readonly >
                        </div>
                        ...
                        <div class="row gx-2 mt-4">
                            <div class="col-3">
                                <select name="etatremise" id="remise" class="form-select">
                                    <option value="off" {{ old('etatremise', $bon->etatremise) == 'off' ? 'selected' : '' }}>Aucune remise</option>
                                    <option value="pourcentage" {{ old('etatremise', $bon->etatremise) == 'pourcentage' ? 'selected' : '' }}>Pourcentage</option>
                                    <option value="montant" {{ old('etatremise', $bon->etatremise) == 'montant' ? 'selected' : '' }}>Montant fixe</option>
                                </select>
                            </div>
                            <div class="col-2">
                                <input type="number" step="1" id="InpRemis" name="remise" 
                                       class="form-control" value="{{ old('remise', $bon->remise) }}" min="0">
                            </div>
                        </div>
                        ...
                        <table class="table mt-4">
                            <tr>
                                ...
                                <td class="border-0 text-end"><span id="montant-total">{{ number_format($bon->montant, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="montant" id="montant" value="{{ number_format($bon->montant, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">Remise :</th>
                                <td class="border-0 text-end"><span id="remis">{{ number_format($bon->remise, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="montantRemise" id="motantRemise" value="{{ number_format($bon->remise, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">TVA :</th>
                                <td class="border-0 text-end"><span id="tvaa">{{ number_format($bon->taux * $bon->montant / 100, 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="valeurTVA" id="motantTVA" value="{{ number_format($bon->taux * $bon->montant / 100, 2, '.', '') }}">
                            </tr>
                            <tr>
                                <th class="border-0 text-end">Total TTC :</th>
                                <td class="border-0 text-end"><span id="montant-ttc">{{ number_format($bon->montant + ($bon->taux * $bon->montant / 100), 2, '.', '') }}</span> DH</td>
                                <input type="hidden" name="totalTTC" id="motantTotal" value="{{ number_format($bon->montant + ($bon->taux * $bon->montant / 100), 2, '.', '') }}">
                            </tr>
                        </table>

                        <!-- Hidden Fields -->
                        <!-- Hidden Fields -->
                        <input type="hidden" name="articles" id="articlesInput" value="{{ old('articles', json_encode($bonArticles)) }}">

                        <div class="text-center mt-5">
                            <a href="{{ route('bon_retour.index') }}" class="btn btn-secondary">Retour</a>
...
<script>
    ...
    let products = @json(old('articles') ? json_decode(old('articles')) : $oldProduct);
    ...
    function renderProductsTable() {
        ...
        products.forEach((product, index) => {
            ...
            const displayPrice = (typeof product.prix === 'number') ? product.prix.toFixed(2) : product.prix;
    ...
    function convertTtcToHt(index, inputElement) {
        ...
        if (ttcValue > 0 ) {
            ...
            const preciseValue = parseFloat(htValue.toFixed(6));
            inputElement.value = htValue.toFixed(2);
            updateProduct(index, 'prix', preciseValue);
        } else {
            // If TVA is not checked or value is 0, use the value as is
    ...
    function prepareForm() {
        ...
        if (remisInput && !remisInput.value.trim()) {
            remisInput.value = '0';
        }
