...
@endphp
            ...
            <div class="col-lg-12">
                <div class="card p-md-3 p-lg-4">
                    <div class="card-body">
                        ...
                        @endif
                        <form id="offreCommerciale" method="POST" action="{{ route('offreCommerciale.update',$OffreCommerciale['id']) }}" class="row g-3">
                            @csrf
                            @method('PUT')

                            <div class="col-md-6">
                                <label for="exampleDataList" class="form-label"> Objectif :</label>
                                <input class="form-control @error('objectif') is-invalid @enderror"
                                list="datalistOptions" id="exampleDataList"
                                value="{{old('objectif')?old('objectif'):$OffreCommerciale['objectif']}}"
                                name="objectif" placeholder="Veuillez saisir ici...">
                        ...
                        @enderror
                            ...
                            <table class="table">
                                <tr>
                                    ...
                                    <input type="hidden" id="montant" name="montant" value="{{ old('montant') ? old('montant') : $OffreCommerciale['montant'] }}">
                                    @error('montant')
                                        <div class="text-danger ps-3">{{ $message }}</div>
                                    @enderror
                                </tr>
                            ...
                            <div class="hiddenarticlesInputs">
                                <input type="hidden" name="articles" id="inputarticlesHiddent">
                            </div>
                            ...
                            <div class="text-center mt-5">
                                ...
                                <button type="submit" class="btn btn-primary" id="submitFFOffreC">Enregister</button>
                            </div>
                        </form>
    ...
    <script>
        ...
        $(document).ready(() => {
            var OffreCommerciale_article = @json($OffreCommerciale_article);
            var Allarticles = @json($articles);
            var OffreCommerciale = @json($OffreCommerciale);
        ...
        function calculateAmount(dataProd){
                ...
                // document.getElementById('tva').textContent = montantTva.toFixed(2);
                }
            }
    ...
    updateMontant(document.querySelector(`#ligne-${rowCount} .qty`), 0);
        ...
        // this for add product in list or add old product 
            if (oldProduct) {
                ...
                OffreCommerciale_article.forEach(elmntFA => {
                        ajouterLigne(elmntFA)
                    });
            }
        // +++++++++
        ...
        const btnajouterLigne = document.getElementById('btnajouterLigne');
            ...
            $('#submitFFOffreC').click(function() {

                // ========

                let inputarticlesHiddent = document.getElementById('inputarticlesHiddent');

                    if (dataProdouits.length > 0) {

                        let dataProdouitsString = JSON.stringify(dataProdouits);
                        inputarticlesHiddent.value = dataProdouitsString;
                    }else{
                        inputarticlesHiddent.value = null ;
                    };
